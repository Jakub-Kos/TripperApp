...
public interface IUserRepository
{
    ...
    Task<UserRecord?> FindById(string userId, CancellationToken ct);
...
public sealed class UserRepository(AppDbContext db) : IUserRepository
{
    public Task<UserRecord?> FindByEmail(string email, CancellationToken ct) =>
        db.Users.Include(u => u.RefreshTokens).FirstOrDefaultAsync(u => u.Email == email, ct);

    public Task<UserRecord?> FindById(string userId, CancellationToken ct) =>
        db.Users.Include(u => u.RefreshTokens).FirstOrDefaultAsync(u => u.UserId == userId, ct);
    ...
    public Task<RefreshTokenRecord?> FindRefreshToken(string token, CancellationToken ct) =>
        db.RefreshTokens.Include(t => t.User).FirstOrDefaultAsync(t => t.Token == token, ct);
